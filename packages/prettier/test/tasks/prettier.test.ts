...
describe('prettier', () => {
  ...
  beforeAll( async () => {
    ...
    formattedDefaultFixture = await fsp.readFile(path.join(testDirectory, './fixtures/formatted-default.ts'), 'utf8');
    formattedConfigFileFixture = await fsp.readFile(path.join(testDirectory, './fixtures/formatted-config-file.ts'), 'utf8');
    formattedConfigOptionsFixture = await fsp.readFile(path.join(testDirectory, './fixtures/formatted-config-options.ts'), 'utf8');
  })
  ...
  it('should format the correct file with default configOptions', async () => {
    ...
    await fsp.writeFile(path.join(testDirectory, './fixtures/formatted.ts'), prettified)
