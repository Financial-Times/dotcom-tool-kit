...
export async function postInstall(logger: Logger): Promise<void> {
  ...
  try {
    ...
    if (
      rawCircleConfig &&
      rawCircleConfig.includes(semVerRegex.source) &&
      rawCircleConfig.startsWith(automatedComment)
    ) {
      ...
      jobs?.items.forEach((jobItem, index) => {
        ...
        if (jobItem.type === 'PLAIN') {
          ...
          const existingFilter: Pair | undefined = job.value.items.filter(
            (item: Pair) => item.key.value === 'filters'
          )[0]
